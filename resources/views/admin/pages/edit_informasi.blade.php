  ...
  <div class="col-lg-12 p-5">
    ...
    <div class="card p-3">
      ...
      <div class="card-body">
        ...
        @endif
        <form action="{{url('/admin/informasi/edit/post')}}" method="POST" enctype="multipart/form-data">
          @csrf
          <div class="mb-3">
          ...
          <div class="mb-5">
            ...
            <input type="text" class="form-control" id="id_informasi" name="id_informasi" value="{{$informasi->id}}">
            <input type="text" class="form-control" id="title" name="title" value="{{$informasi->judul}}">
          </div>
          <div class="mb-5 ">
            <label for="image" class="form-label">Image</label>
            <input type="file" class="form-control-file" id="image" accept="image/*" name="image" onchange="loadFile(event)">
            <img id="output" class="rounded border-0" style="max-width: 400px; max-height: 300px;" src="{{asset('informasi')}}/{{$informasi->img}}">
          </div>
          <div class="mb-3">
            <!-- <label for="desc" class="form-label">Desc</label> -->
            <textarea name="desc" class="my-editor form-control" id="my-editor" cols="50" rows="50">{{$informasi->description}}</textarea>
          </div>
          <button type="submit" class="btn btn-success btn-post"><i class="fa-brands fa-usps me-2"></i>Submit</button>
          <a href="{{ url('/admin/daftar/informasi') }}" class="btn btn-danger btn-cencel">Batal</a>
        </form>
      </div>
    </div>
  </div>
...
<script>
  ...
  $(document).ready(function() {
    ...
    if (message) {
      swal("Success!", "Informasi Berhasil di update", "success")
    }
  });
